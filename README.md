...
Повторное нажатие правой Alt с тем же предметом удаляет только этот запрос. Если предмета уже нет в руке, откройте поиск пустой рукой: активные запросы показаны первыми с отметкой `✓`, и щелчок по активному запросу удаляет его. Кнопка **«Очистить поиск»** отключает все запросы сразу.
